...
const LoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!result.success) {
        ...
        const targetPath = result.user.role === 'admin' ? '/admin-panel/advisories' : '/dashboard';
        navigate(targetPath, { replace: true });
      }
    } catch (error) {
      toast({
  ...
  return (
    <>
      <Helmet>
        <title>Iniciar Sesión - Gsus Martz</title>
        <meta name="description" content="Accede a tu plataforma de entrenamiento personal con Gsus Martz" />
      </Helmet>
        ...
        <div className="w-full max-w-md">
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <Button type="submit" disabled={isLoading || authLoading} className="w-full btn-primary text-white bg-gray-900 border border-gray-700 hover:bg-gray-800 hover:text-white">
                {isLoading || authLoading ? <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white"></div> : 'Iniciar Sesión'}
              </Button>
            </form>

            <div className="relative my-6">
              <div className="absolute inset-0 flex items-center"><span className="w-full border-t border-gray-600"></span></div>
              <div className="relative flex justify-center text-sm"><span className="bg-[#282d34] px-2 text-gray-400">O continúa con</span></div>
            </div>
            ...
            <div className="mt-8 text-center text-sm text-gray-400 space-y-2">
              <p>
                ...
                <Link to="/signup" className="font-medium text-[#5ebe7d] hover:underline">
                  Regístrate
                </Link>
              </p>
              <p>
                ¿Has olvidado tu contraseña?{' '}
                <Link to="/reset-password" className="font-medium text-[#5ebe7d] hover:underline">
                  Recupérala
                </Link>
              </p>
            </div>
