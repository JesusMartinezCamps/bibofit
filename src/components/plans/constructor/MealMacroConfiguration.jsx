...
const MacroSlider = React.memo(({ label, colorClass, value, grams, onChange, disabled, onDisabledClick }) => {
  ...
  return (
    <div className="w-full min-w-[100px] flex flex-col gap-1.5" onClick={disabled ? onDisabledClick : undefined}>
      ...
      <div className={cn(disabled && "cursor-not-allowed")}>
          <Slider
              ...
              disabled={disabled}
              className={cn("py-1 relative macro-slider", disabled && "opacity-50 pointer-events-none")}
...
const MealMacroConfiguration = ({ 
  ...
  }, [canUseAutoFrame, autoFrameMessage, autoFrameLink, toast]);
  ...
  const handleMealPctChange = useCallback((mealId, macro, value) => {
    if (readOnly) {
         toast({
            ...
            description: "Mejora tu plan para poder usar esta función.",
            variant: "destructive"
        });
        return;
    }
            ...
            >
              <div>
                <CardTitle className="flex items-center gap-2">
                    ...
                    {(readOnly || !canUseAutoFrame) && <Lock className="w-4 h-4 text-gray-500" />}
                </CardTitle>
          ...
          <CardContent className="px-0 sm:px-6 pb-4">
            ...
            {!hideSaveButton && (
                <CardFooter className="flex justify-end mt-4 p-4 bg-slate-900/70 border-t border-gray-800 rounded-b-lg">
                    {readOnly || !canUseAutoFrame ? (
                        <div className="text-sm text-gray-400 flex items-center gap-2 italic">
                            <Lock className="w-3 h-3"/> {readOnly ? "Función reservada para planes premium" : "Actualiza a Premium para guardar"}
                        </div>
                    ) : (
