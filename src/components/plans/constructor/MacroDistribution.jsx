import React, { useMemo, useState, useEffect, useRef } from 'react';
    ...
    const MacroInput = ({ value, onChange, icon, label, colorClass, grams }) => (
        <div className="flex items-center justify-between gap-2 bg-gray-800/60 p-2 rounded-md border border-gray-700">
    ...
    const MacroDistribution = ({ effectiveTdee, macrosPct: initialMacrosPct, onMacrosPctChange, calorieOverrides, dietPlanId, onOverridesUpdate, isTemplate }) => {
        ...
        return (
            ...
            <Collapsible open={open} onOpenChange={setOpen}>
                ...
                <CollapsibleTrigger asChild>
                    ...
                    >
                    <div>
                        <CardTitle>1. Reparto de Macros Diarios</CardTitle>
                        <CardDescription>
                        Ajusta la distribución porcentual y revisa los gramos resultantes.
                        </CardDescription>
                    </div>
                ...
                {open && (
                    ...
                    <CardContent className="px-0 pb-0">
                        {!isTemplate && (
                            <CalorieAdjustment 
                                calculatedTdee={effectiveTdee}
                                calorieOverrides={calorieOverrides}
                                dietPlanId={dietPlanId}
                                onOverridesUpdate={onOverridesUpdate}
                            />
                        )}
                        <div className="rounded-lg p-4 border border-gray-700 space-y-4 mt-4">
            ...
            </AnimatePresence>   {/* ✔ CIERRE QUE FALTABA */}
        </Collapsible>            {/* ✔ AHORA SÍ SE CIERRA AQUÍ */}
    </Card>
        );
    };
