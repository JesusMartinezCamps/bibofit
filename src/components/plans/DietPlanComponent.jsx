...
const DietPlanComponent = () => {
  ...
  const [shoppingListInitialMode, setShoppingListInitialMode] = useState('day');
  const [shoppingListInitialDate, setShoppingListInitialDate] = useState(new Date());
    ...
    try {
      ...
      if (dietPlan) {
        const { data: overridesData, error: overridesError } = await supabase
          ...
          .select('effective_date, manual_calories')
  ...
  const handleShoppingListClick = () => {
    setShoppingListInitialMode(viewMode === 'list' ? 'day' : 'week');
    setShoppingListInitialDate(currentDate);
...
const calculateTargetMacros = useCallback(() => {
    ...
    const applicableOverride = data.calorieOverrides
        .filter(o => o.effective_date <= logDate)
...
const combinedPlanRestrictions = useMemo(() => {
  ...
  return (
    <div className="overflow-x-clip">
      ...
      />
      <GlobalShoppingListDialog
        open={isShoppingListOpen}
        onOpenChange={setIsShoppingListOpen}
        initialMode={shoppingListInitialMode}
        initialDate={shoppingListInitialDate}
      />
      <ReminderFormDialog
