...
const PersonalDataForm = ({ className, onSave, userId: propUserId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData(prev => ({ ...prev, height_cm: heightCm ? heightCm.toString() : '' }));
      setInitialFormData({ ...formData, height_cm: heightCm ? heightCm.toString() : '' });

      if (metabolismResult.success) {
        if (metabolismResult.missingData) {
          toast({ 
            title: 'Datos guardados', 
            description: `Datos personales guardados. ${metabolismResult.missingData}`,
            variant: 'default'
          });
        } else {
          toast({ 
            title: 'Éxito', 
            description: 'Datos personales guardados y metabolismo calculado correctamente.' 
          });
        }
      } else {
        toast({ 
          title: 'Datos guardados', 
          description: 'Datos personales guardados, pero no se pudo calcular el metabolismo.',
          variant: 'default'
        });
      }
