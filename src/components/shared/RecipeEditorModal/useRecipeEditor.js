...
import { submitChangeRequest, savePrivateRecipe, saveFreeRecipe, saveDietPlanRecipe, updateRecipeDetails } from './recipeService';
import { v4 as uuidv4 } from 'uuid';
import { supabase } from '@/lib/supabaseClient';


export const useRecipeEditor = ({ recipeToEdit, onSaveSuccess, isAdminView, userId: propUserId, open, planRestrictions: initialPlanRestrictions, isTemporaryEdit = false, initialConflicts = null, allFoods }) => {
  const { user } = useAuth();
  ...
  const handleSubmit = useCallback(async (actionType = 'save') => {
    ...
    try {
        ...
        // Standard update for existing recipes that support simple updates
        if (isSimpleUpdate && recipeToEdit.id && recipeToEdit.type !== 'recipe') {
        ...
        if (recipeToEdit.type === 'private_recipe' || recipeToEdit.is_private_recipe) {
            ...
            if (isAdminView) {
                // Modified: ALWAYS create a new recipe variant in recipes table if we are resolving a conflict
                // or if specifically requested to create a variant.
                // However, `saveDietPlanRecipe` currently handles creation of `diet_plan_recipes` entry
                // which might be `is_customized=true`. The user specifically asked to create a record in 
                // `recipes` table first.
                // This logic is now handled in `AdminRecipeModal` for the "Variant Creation" flow.
                // But if we fall here for standard saving:
                ...
                // For normal saves (not creating a full template variant via the dedicated button in modal),
                // we stick to creating a customized plan recipe.
...
}, [hasChanges, formData, ingredients, originalIngredients, recipeToEdit, userId, onSaveSuccess, toast, isTemporaryEdit, isAdminView, initialFormData, hasIngredientChanges]);
