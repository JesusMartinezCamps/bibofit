...
const RecipeEditorModal = ({ open, onOpenChange, recipeToEdit, onSaveSuccess, isAdminView, userId, planRestrictions, initialConflicts = null, adjustments = null }) => {
  const { user } = useAuth();
  ...
  const isEditingMode = mode === 'settings' && isEditable;
  ...
  const toggleSwitchColor = isFreeRecipe ? "data-[state=checked]:bg-sky-400" : "data-[state=checked]:bg-green-500";
  const activeIconColor = isFreeRecipe ? "text-sky-400" : "text-green-500";
  ...
  const saveButtonText = useMemo(() => {
      ...
      if (hasChanges && (hasInitialConflicts || hasIngredientChanges)) {
          return "Crear variante sin conflictos";
      }
      
      if (!hasChanges) return "Sin cambios";
      
      return hasIngredientChanges ? "Guardar como nueva receta" : "Guardar cambios";
  }, [hasCriticalConflicts, hasChanges, hasInitialConflicts, hasIngredientChanges]);
              ...
              <div ref={ingredientsContainerRef} className="flex-1 overflow-y-auto styled-scrollbar-green">
                {isSearching ? (
                  ...
                  <RecipeView
                    ...
                    disableAutoBalance={!hasIngredientChanges}
                    enableStickyMacros={true}
                  />
                )}
                  
                  {!isSearching && isEditingMode && (
