...
export const applyTemporaryChanges = async ({ formData, ingredients, recipeToEdit }) => {
    try {
        const updatedRecipeData = {
            ...
            name: formData.name, // for private recipes
            custom_instructions: formData.instructions,
            instructions: formData.instructions, // for private recipes
            custom_prep_time_min: formData.prep_time_min,
            prep_time_min: formData.prep_time_min, // for private recipes
            custom_difficulty: formData.difficulty,
            difficulty: formData.difficulty, // for private recipes
            is_customized: true,
            custom_ingredients: ingredients.map(ing => ({
            ...
            // Esto es un cambio temporal en el front, no se guarda en BD
            recipe: {
                ...recipeToEdit.recipe,
                name: formData.name,
            ...
            private_recipe_ingredients: ingredients.map(ing => ({ // for private recipes
                food_id: ing.food_id,
                user_created_food_id: ing.user_created_food_id,
                grams: ing.grams || ing.quantity || 0,
                food: ing.food,
            }))
        };
...
export const submitChangeRequest = async ({ actionType, recipeToEdit, formData, ingredients, originalIngredients, userId }) => {
    try {
        ...
        if (ingredientsError) {
            ...
            // Optional: Clean up the created private_recipe if ingredients fail
        ...
        // 3. Create the change request record, linking to the new private recipe
        const changeRequest = {
            user_id: userId,
            status: 'pending',
        ...
        if (requestError) {
            ...
            // Optional: Clean up created private recipe and ingredients
...
export const updateDietPlanRecipeCustomization = async ({ dietPlanRecipeId, formData, ingredients }) => {
    try {
        // 1. Update the diet_plan_recipes row to mark as customized and save text fields
        ...
        // 2. Delete existing custom ingredients for this plan recipe
        ...
        // 4. Update Macros if needed (Optional but recommended to keep cache fresh)
        // Note: Macros are often recalculated on fly, but `recipe_macros` table exists for caching plan recipe macros
        // We should recalculate based on ingredients here, but for now we skip complex calculation
        // assuming frontend or next fetch handles it, or implement simple sum if critical.
    ...
    try {
        ...
        const { data: newRecipe, error: recipeInsertError } = await supabase
            ...
            .insert({
                ...
                parent_private_recipe_id: recipeId, // Link to original
...
export const saveDietPlanRecipe = async ({ recipeId, userId, formData, ingredients, originalRecipe, isNew = false }) => {
    try {
        // This function is generally used for creating NEW instances in a plan (e.g. from template/global)
        // For updating existing assignments in place, use updateDietPlanRecipeCustomization.
        
        let newRecipeId;
        
        const payload = {
            diet_plan_id: originalRecipe.diet_plan_id || originalRecipe.dietPlanId,
            recipe_id: originalRecipe.recipe_id || originalRecipe.recipeTemplateId || originalRecipe.id, // Original global recipe ID
            ...
            custom_prep_time_min: formData.prep_time_min,
            custom_difficulty: formData.difficulty,
            parent_diet_plan_recipe_id: originalRecipe.diet_plan_recipe_id // Link if available
        };
        ...
        return { 
            ...
            data: newRecipe,
            action: 'variant_created' // Signal to UI to refresh list or replace item
        };
