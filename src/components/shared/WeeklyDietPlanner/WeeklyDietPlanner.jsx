...
const WeeklyDietPlanner = forwardRef(({ isAdminView, userId, viewMode = 'week', logDate: propLogDate, currentDate, activePlan, onAddRecipeClick, onPlanUpdate, plannedMeals, setPlannedMeals, userRestrictions }, ref) => {
    ...
    useImperativeHandle(ref, () => ({
        ...
        scrollToDay(date) {
            ...
            if (dayIndex !== -1 && dayElementsRef.current && dayElementsRef.current.children[dayIndex]) {
                dayElementsRef.current.children[dayIndex].scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'center' });
            }
        },
    }));
    ...
    const handleSnackUpdate = useCallback((newLog, newSnackWithOccurrence) => {
        ...
        setSnacks(prev => {
            const existingIndex = prev.findIndex(s => s.occurrence_id === snackWithUnifiedIngredients.occurrence_id);
            if (existingIndex > -1) {
                const updated = [...prev];
