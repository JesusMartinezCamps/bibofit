...
const RecipeCard = ({
  ...
  const recipeName = useMemo(() => {
    ...
    return recipe.custom_name || recipe.recipe?.name || "Receta";
  }, [recipe]);
  ...
  const recipeIngredients = useMemo(() => {
    ...
    // If it's a template or plan recipe that hasn't been customized yet, fall back to base recipe ingredients.
  ...
  const macros = useMemo(() => {
    if (!adjustedIngredients || !allFoods) {
  ...
  if (isListView) {
    return (
      <div className="relative group h-full">
        <button onClick={() => handleRecipeClick({ ...recipe, is_private_recipe: isPrivate }, adjustment)} className={cn(
  ...
  return (
    ...
    >
      <button onClick={() => handleRecipeClick({ ...recipe, is_private_recipe: isPrivate }, adjustment)} className="flex-1 text-left p-3 w-4/5">
