  ...
  const fetchEventsAndReminders = useCallback(async () => {
    ...
    try {
      ...
      const [workoutsRes, weightLogsRes, advisoriesRes, snackLogsRes, remindersRes] = await Promise.all([
        ...
        isManagerView ? supabase.from('reminders').select('*').eq('user_id', userId).eq('type', 'event') : Promise.resolve(null)
      ]);
      ...
      const addEvent = (date, event) => {
        ...
        const idx = formattedEvents[key].findIndex(e => e.type === event.type);
        if (idx === -1) formattedEvents[key].push(event);
        else if (event.type === 'diet_log') formattedEvents[key][idx] = { ...formattedEvents[key][idx], ...event };
      };
      ...
      if (snackLogsRes.data) {
        snackLogsRes.data.forEach(log => {
          ...
          if (exist) exist.hasSnack = true;
          else addEvent(log.log_date, { title: `0/${totalSlots}`, type: 'diet_log', hasSnack: true });
        });
      }
...
<div className={cn("w-full h-full flex flex-col", isClientView && "sm:p-6 sm:bg-[#1a1e23] sm:rounded-2xl")}>
  ...
  <div className="grid grid-cols-7 flex-grow">
    {calendarDays.map((day) => {
      ...
      <div
        key={day.toString()}
        className="border border-gray-800 p-1 relative cursor-pointer hover:bg-[#2a2f36] min-h-[114px] flex flex-col"
                    ...
                    >
                      ...
                      <TooltipContent className="bg-slate-800 text-white border-amber-400 max-w-sm p-3 space-y-2">
                        ...
                        <div className="text-xs text-amber-200/90 space-y-1 pt-2 border-t border-amber-500/20">
                          ...
                          {rem.category && <p>Categoría: {rem.category}</p>}
                        </div>
                      </TooltipContent>
                    </Tooltip>
                  );
          ...
          <div className="space-y-1">
            {dayEvents.map((event, index) => {
              ...
              if (event.type === 'diet_log') {
                ...
                if (done === total) fromColor = 'rgb(9 147 60 / 82%)';
                else if (done >= total / 2) fromColor = 'rgb(34 197 94 / 20%)';
