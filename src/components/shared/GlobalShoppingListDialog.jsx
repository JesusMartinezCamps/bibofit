...
const HighlightedText = ({ text, highlight }) => {
    ...
    // Normalize for comparison but keep original text for display
    ...
    // This simple split/join won't work well with accents normalization for display preservation
    // A more robust approach requires finding indices
    const parts = [];
    let currentIndex = 0;
    ...
    while (searchIndex !== -1) {
        ...
        // Add matched text (using original string length is tricky if accents removed change length, 
        // but for simple removing diacritics usually length is preserved or we accept slight mismatch in edge cases.
        // Better approach: standard regex with case insensitive flag if no accent normalization needed, 
        // but since we normalize, let's try a regex on the original text that ignores accents if possible,
        // or just basic case insensitive match if normalization is too complex for UI highlighting)
        
        // Let's stick to a simpler Case-Insensitive highlight for now which is safer and common
        // If we really need accent-insensitive highlighting on original text it gets complex.
        // For this requirement, usually case-insensitive matching of the typed query is what's expected.
        
        // If we want strict accent-insensitive highlighting, we have to map indices.
        // Given the user request "highlight the matching text", let's do a regex match based on the search term.
        // If the user types "limon", and text is "Limón", strict regex won't match. 
        // Let's use a simpler approach: 
        // 1. Escape special regex chars in highlight
        // 2. Create a regex that matches the highlighting text
        
        // Fallback to simple Case-Insensitive for display stability
            ...
            <>
                {splitParts.map((part, i) => 
                    part.toLowerCase() === highlight.toLowerCase() ? (
                        <span key={i} className="bg-yellow-500/40 text-yellow-100 font-bold rounded px-0.5 shadow-[0_0_10px_rgba(234,179,8,0.2)]">{part}</span>
                    ) : (
                        // If strict equality fails (e.g. accents), try to see if it was the captured group
                        // The regex split keeps delimiters (captured groups).
                        // If the regex matched, it means this part matches the search term case-insensitively.
                        // However, split() behavior with capturing groups includes the match.
                        // We need to verify if this part is indeed a match.
                        regex.test(part) ? (
                             <span key={i} className="bg-yellow-500/40 text-yellow-100 font-bold rounded px-0.5 shadow-[0_0_10px_rgba(234,179,8,0.2)]">{part}</span>
                        ) : (
                            part
...
// Specialized component for accent-insensitive highlighting
const SmartHighlight = ({ text, highlight }) => {
    ...
    // We need to rebuild the string with highlights while preserving original characters
    const result = [];
...
const ShoppingListGroup = ({ title, icon, items, checkedItems, onCheckedChange, searchQuery }) => {
    const sortedItems = useMemo(() => {
        return [...items].sort((a, b) => {
            // Provide a fallback to prevent crash if checkedItems is somehow undefined during initial render cycles,
            // although the main fix in the parent component should prevent this.
                    ...
                    return (
                        ...
                        >
                            ...
                            <div className="flex-1 pointer-events-none">
                                ...
                                {/* Only show single recipe name if not aggregated */}
                                {!item.recipeCounts && item.recipeName && (
                                    <p className="text-xs text-gray-400 mt-1 italic pl-1">
                                        Para: <SmartHighlight text={item.recipeName} highlight={searchQuery} />
                                    </p>
                                )}
                                {/* Show aggregated recipe names for Complete mode deduplicated items */}
...
const GlobalShoppingListDialog = ({ open, onOpenChange, initialMode = 'planned', initialDate, fromHeader = false }) => {
    ...
    const defaultMode = initialMode === 'week' ? 'planned' : (initialMode === 'day' ? 'complete' : (initialMode || 'planned'));
    const [listMode, setListMode] = useState(defaultMode);
    ...
    const syncShoppingList = useCallback(async () => {
        ...
        allItems.forEach(item => {
            const itemId = item.is_user_created ? `uc_${item.id}` : `std_${item.id}`;
    ...
    useEffect(() => {
        if (open) {
             const mode = initialMode === 'week' ? 'planned' : (initialMode === 'day' ? 'complete' : (initialMode || 'planned'));
            setListMode(mode);
        }
    }, [open, initialMode]);
    ...
    const getMacroCategory = (food) => {
        ...
        // STRICT mapping for carbs including the specific "carbs" key used in DB
    ...
    const fetchShoppingData = useCallback(async () => {
        ...
        try {
            ...
            const completeModeAggregationMap = new Map(); // For Complete mode deduplication
            const ingredientsMap = new Map(); // For Planned mode aggregation
            let recipeNamesMap = new Map(); // For tracking recipe sources
            ...
            allMealsToProcess.forEach(recipe => {
                ...
                ingredients.forEach(ing => {
                    ...
                    if (listMode === 'planned') {
                        ...
                        // Complete mode deduplication
                        if (completeModeAggregationMap.has(food.id)) {
                            // Aggregating recipe names but keep one entry
                            const existing = completeModeAggregationMap.get(food.id);
                            // Just collect recipe names if not already present
                            if (!existing.recipeNames.includes(recipeName)) {
                             ...
                             completeModeAggregationMap.set(food.id, {
                                ...food,
                                displayQuantity: null, // We don't sum for complete mode view in terms of total quantity per line, but just listing the food once
                                unit: food.food_unit === 'unidades' ? 'ud(s)' : 'g',
            ...
            if (listMode === 'planned') {
                Array.from(ingredientsMap.values()).forEach(food => {
                 ...
                 // Flatten complete mode aggregation
    ...
    const filteredData = useMemo(() => {
        ...
        const filterItemList = (items) => {
            return items.filter(item => {
                ...
                // Match Recipe Names (Planned Mode)
                ...
                // Match Recipe Name (Complete Mode)
