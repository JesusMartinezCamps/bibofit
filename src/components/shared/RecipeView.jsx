...
const IngredientCard = ({
  ...
  const getStatusDisplay = (type, conflicts, recommendations) => {
    ...
    // Fallback for editing mode or if we want to show conflicts below in some cases
  ...
  // For display in view mode, use parsed number. 
  // For edit mode (input), we use the raw quantity passed down which might be string.
    ...
    <div data-ingredient-food-id={food.id} className={cn("relative p-3 rounded-lg border transition-all duration-300", statusColorClasses.split(' ').filter(c => c.startsWith('bg-') || c.startsWith('border-')))}>
      ...
      {isEditing && (vitamins.length > 0 || minerals.length > 0) && (
        <div className="mt-2 pt-2 border-t border-slate-700/50 flex flex-wrap gap-1.5">
          {/* Removed 'Micronutrientes Clave' header, just showing badges directly */}
          {vitamins.map(v => <NutrientBadge key={`v-${v.id}`} nutrient={v} />)}
          {minerals.map(m => <NutrientBadge key={`m-${m.id}`} nutrient={m} />)}
        </div>
      )}
  ...
  <TooltipProvider>
    <Tooltip>
      <TooltipTrigger asChild>
        <div className="px-2 py-0.5 text-xs rounded-full bg-purple-500/20 text-purple-300 border border-purple-500/30 cursor-default">
          {nutrient.name}
        </div>
      </TooltipTrigger>
      <TooltipContent className="bg-gray-800 border-purple-700 text-white max-w-xs">
        <p className="font-bold text-purple-300">{nutrient.name}</p>
        <p className="text-sm">{nutrient.key_function || nutrient.description}</p>
      </TooltipContent>
    </Tooltip>
  </TooltipProvider>
);
...
const RecipeView = ({
  ...
  useEffect(() => {
    ...
    fetchTargets();
  }, [mealTargetMacros, isEditing, user, recipe]);
  ...
  const getConflictTypeForFood = (food) => {
    ...
    if (recommendation) return 'condition_recommend'; // Map preferred to this as well for display logic if needed, or update IngredientCard
  ...
  const ingredientsWithDetails = useMemo(() => {
    ...
    return recipe.ingredients.map(ing => {
      ...
      // Sanitize quantity for macro calculation (strings/empty become 0 for calculation)
      ...
      return {
        ...
        vitamins,
        minerals,
        quantity: (ing.grams !== undefined && ing.grams !== null) ? ing.grams : ing.quantity, // Keep original string value if editing
        conflictType,
  ...
  const groupedAndSortedIngredients = useMemo(() => {
    // Sort alphabetically by food name to prevent jumping when grams change
  ...
  const renderTotalMacros = (macros, isTotal = false) => (
    <div className={`flex items-center font-numeric ${isTotal ? 'justify-around text-base md:text-lg py-3 px-2 bg-slate-800/50 rounded-lg' : 'justify-end gap-x-4 text-sm'}`}>
      <div className="flex items-center text-orange-400" title="Calorías">
        <CaloriesIcon className="w-4 h-4 mr-1.5" />
        <span>{Math.round(macros.calories)}</span>
        <span className="text-xs text-orange-400/80 ml-1">kcal</span>
      </div>
      <div className="flex items-center text-red-400" title="Proteínas">
        <ProteinIcon className="w-4 h-4 mr-1.5" />
        <span>{Math.round(macros.proteins)}</span>
        <span className="text-xs text-red-400/80 ml-1">g</span>
      </div>
      <div className="flex items-center text-yellow-400" title="Carbohidratos">
        <CarbsIcon className="w-4 h-4 mr-1.5" />
        <span>{Math.round(macros.carbs)}</span>
        <span className="text-xs text-yellow-400/80 ml-1">g</span>
      </div>
      <div className="flex items-center text-green-400" title="Grasas">
        <FatsIcon className="w-4 h-4 mr-1.5" />
        <span>{Math.round(macros.fats)}</span>
        <span className="text-xs text-green-400/80 ml-1">g</span>
      </div>
    </div>
  );
  ...
  const renderTargetMacros = (targets) => {
    ...
    // Normalize targets object structure
    const t = {
      calories: targets.target_calories ?? targets.calories ?? 0,
      proteins: targets.target_proteins ?? targets.proteins ?? 0,
      carbs: targets.target_carbs ?? targets.carbs ?? 0,
      fats: targets.target_fats ?? targets.fats ?? 0
    };

    return (
      <div className="flex items-center font-numeric justify-around text-sm py-2 px-2 bg-slate-900/50 rounded-lg border border-slate-800 border-dashed text-gray-400">
        <div className="flex items-center" title="Objetivo Calorías">
          <CaloriesIcon className="w-3.5 h-3.5 mr-1.5 opacity-70" />
          <span>{Math.round(t.calories)}</span>
          <span className="text-[10px] ml-1 opacity-70">kcal</span>
        </div>
        <div className="flex items-center" title="Objetivo Proteínas">
          <ProteinIcon className="w-3.5 h-3.5 mr-1.5 opacity-70" />
          <span>{Math.round(t.proteins)}</span>
          <span className="text-[10px] ml-1 opacity-70">g</span>
        </div>
        <div className="flex items-center" title="Objetivo Carbohidratos">
          <CarbsIcon className="w-3.5 h-3.5 mr-1.5 opacity-70" />
          <span>{Math.round(t.carbs)}</span>
          <span className="text-[10px] ml-1 opacity-70">g</span>
        </div>
        <div className="flex items-center" title="Objetivo Grasas">
          <FatsIcon className="w-3.5 h-3.5 mr-1.5 opacity-70" />
          <span>{Math.round(t.fats)}</span>
          <span className="text-[10px] ml-1 opacity-70">g</span>
        </div>
      </div>
    );
  ...
  const handleAutoBalance = async () => {
    ...
    if (!activeTargets || [activeTargets.target_proteins, activeTargets.target_carbs, activeTargets.target_fats].some(t => t === 0 || t === null || t === undefined)) {
      toast({
        ...
        description: 'No se encontraron objetivos de macros válidos para esta comida. Asegúrate de que la comida tiene objetivos nutricionales asignados.',
        variant: 'destructive',
      });
      return;
    }
    ...
    setIsBalancing(true);
    try {
      // Prepare ingredients for edge function
      ...
      if (data.balancedIngredients) {
        // Update ingredients with new quantities, preserving other properties
        const newIngredients = recipe.ingredients.map(ing => {
          const balanced = data.balancedIngredients.find(b => String(b.food_id) === String(ing.food_id || ing.food?.id));
          if (balanced) {
            return {
              ...ing,
              grams: balanced.quantity,
              quantity: balanced.quantity
            };
          }
          return ing;
        });
      ...
      toast({
        ...
        description: error.message || 'Ha ocurrido un error al intentar cuadrar los macros.',
        variant: 'destructive',
      });
    } finally {
  ...
  const handleReplaceSelection = (newFoodData) => {
    ...
    const newIngredient = {
      local_id: crypto.randomUUID(), // New identity
      ...
      // Include any other necessary properties from newFoodData
    ...
    toast({
      title: "Ingrediente reemplazado",
      description: `Se ha sustituido por ${newFoodData.food_name}.`
    });
  };
  ...
  // Determine if we should show the auto-balance button internally
  const showAutoBalance = isEditing && recipe.ingredients?.length > 0 && (mealTargetMacros || recipe.day_meal_id);

  // Render Replacement View if active
  if (replacingIngredient) {
    return (
      <div className="h-full p-4 md:p-6">
        <IngredientSearch
          selectedIngredients={recipe.ingredients} // Pass current list so we don't pick duplicates if desired
  ...
  return (
    <div className="text-white space-y-6 p-2 sm:p-4 md:p-6">
        ...
        {isEditing ? (
          ...
          <>
            ...
            <div className="flex justify-center gap-3 mt-2">
              {greenCount > 0 && (
                <span className="flex items-center text-sm text-green-400 gap-1.5 bg-green-900/20 px-2 py-1 rounded-full border border-green-500/30" title={`${greenCount} alimentos preferidos/recomendados`}>
                  <ThumbsUp className="w-4 h-4" /> {greenCount} Recomendados
                </span>
              )}
              {redCount > 0 && (
                <span className="flex items-center text-sm text-red-400 gap-1.5 bg-red-900/20 px-2 py-1 rounded-full border border-red-500/30" title={`${redCount} alimentos a evitar`}>
      ...
      <div className={cn("grid grid-cols-2 gap-4 rounded-lg", isEditing ? "sm:p-0.5" : "p-3 bg-slate-800/50")}>
      ...
      <div className={cn(
        enableStickyMacros && "sticky top-0 z-30 bg-[#0C101D] -mx-2 px-2 sm:-mx-4 sm:px-4 md:-mx-6 md:px-6 py-2 shadow-xl border-b border-gray-800/60 mb-4"
      )}>
        ...
        {isEditing && (mealTargetMacros || fetchedTargets) && (
          <div className="mt-4">
      ...
      <div>
        ...
        <div className="space-y-4">
          {isEditing ? (
            ...
            // View mode (flat list as well, but with different visual style via displayAsBullet)
              ...
              {groupedAndSortedIngredients.length > 0 ? (
                groupedAndSortedIngredients.map((ing, index) => (
                  <IngredientCard
                    ...
                    allFoodGroups={allFoodGroups}
                    // Fix: Pass popover props for View mode
                    popoverId={ing.local_id || `${ing.food_id}-${index}`}
                    isOpen={openIngredientPopoverId === (ing.local_id || `${ing.food_id}-${index}`)}
                    onOpenChange={handleIngredientPopoverChange}
                  />
