...
const CreateFoodForm = ({ foodToEdit, onFoodActionComplete, isEditing, isClientRequest }) => {
  ...
  const renderCollapsibleSection = (id, title, icon, borderColor, children) => (
    <Collapsible open={openSections[id]} onOpenChange={() => toggleSection(id)} className={cn('w-full p-4 rounded-lg border', borderColor)}>
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className={cn(isReadOnly && "pointer-events-none opacity-80")}>
          ...
          {renderCollapsibleSection('macros', 'Macronutrientes', <Scale className="h-6 w-6 text-cyan-400" />, 'border-cyan-500/50',
            <>
              <FoodFormSection 
