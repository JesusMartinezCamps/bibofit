  ...
  useEffect(() => {
    if (selectedRecipe) {
      reset({
        ...
        difficulty: selectedRecipe.difficulty || '', // Ensure this matches the select values exactly
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (selectedRecipe) {
        ...
        // For new recipes, get current user ID for created_by
      ...
      // 3. Update Sensitivities (Derived from ingredients)
      ...
      if (uniqueSensitivityIds.size > 0) {
        ...
        const { error: sensError } = await supabase.from('recipe_sensitivities').insert(sensitivitiesToInsert);
        if (sensError) throw sensError;
      }

      // 4. Update Macros Cache
      ...
      toast({
        title: '¡Éxito!',
        description: `Plantilla de receta ${selectedRecipe ? 'actualizada' : 'creada'} correctamente.`,
      });
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            ...
            <Input id="name" {...register('name')} placeholder="Ej: Pollo con arroz" className="input-field" />
            {errors.name && <p className="text-red-500 text-xs mt-1">{errors.name.message}</p>}
          </div>
          <div className="space-y-2">
            <Label htmlFor="prep_time_min">Tiempo (min)</Label>
            <Input id="prep_time_min" type="number" {...register('prep_time_min')} placeholder="Ej: 30" className="input-field" />
          ...
          <div className="space-y-2 md:col-span-2">
            ...
            <Textarea id="instructions" {...register('instructions')} rows={5} placeholder="Describe los pasos de la receta..." className="input-field" />
          </div>
        </div>
        ...
        <div className="flex justify-end">
          <Button type="submit" disabled={isSubmitting} className="bg-green-600 hover:bg-green-700 text-white">
            ...
            {selectedRecipe ? 'Guardar Cambios' : 'Crear Plantilla'}
          </Button>
        </div>
      </form>
    </FormProvider>
