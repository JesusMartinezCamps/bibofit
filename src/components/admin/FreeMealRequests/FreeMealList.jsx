      ...
      const handleAction = async (actionType, recipe, data) => {
        try {
            if (actionType === 'approve_private') {
                ...
                if (error) throw error;
                toast({ title: 'Éxito', description: 'Receta privada creada y asignada al plan del cliente.' });
                ...
                const { data: rpcData, error } = await supabase.rpc('add_global_recipe_to_plan', {
                    p_user_id: recipe.user_id,
                    p_day_meal_id: recipe.day_meal_id,
                    p_recipe_name: data.name,
                    p_instructions: data.instructions,
                    p_prep_time_min: data.prep_time_min,
                    p_difficulty: data.difficulty,
                    p_ingredients: ingredientsForRpc,
                });
                if (error || (rpcData && !rpcData.success)) throw new Error(error?.message || rpcData?.error);

                await supabase.from('free_recipes').update({ status: 'approved_general' }).eq('id', recipe.id);
                toast({ title: 'Éxito', description: 'La receta libre ha sido guardada como plantilla de receta general.' });
