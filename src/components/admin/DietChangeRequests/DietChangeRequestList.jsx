import React, { useState, useMemo, useEffect } from 'react';
    import { Loader2, MailQuestion, Inbox, CheckCircle } from 'lucide-react';
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
          <DialogContent className="bg-gradient-to-br from-[#1a1e23] to-[#101418] border-gray-700 text-white max-w-4xl max-h-[90vh] flex flex-col">
            ...
            <div className="flex-grow overflow-y-auto pr-4 styled-scrollbar-green space-y-6">
              ...
              <div>
                <h3 className="font-semibold text-xl text-yellow-400 border-b border-yellow-800 pb-2 mb-4">Nombre de la Receta</h3>
                <div className="px-1">
            ...
            <DialogFooter className="mt-6 gap-2 flex-shrink-0 flex-col sm:flex-row">
              ...
              <Button variant="outline" onClick={() => onAction(request, 'approved', 'save_copy', recipeName)} className="border-green-500 text-green-500 hover:bg-green-500/10 hover:text-green-400">
                  <Copy className="mr-2 h-4 w-4" /> Aceptar y Guardar Copia
              </Button>
              <Button onClick={() => onAction(request, 'approved', 'replace', recipeName)} className="bg-green-600 hover:bg-green-700">
        ...
        try {
            ...
            if (status === 'rejected') {
                ...
                if (approvalType === 'replace') {
                    ...
                    // 2. Find all meal logs for the original recipe
                    let mealLogsQuery;
                    if (request.private_recipe_id) {
                        mealLogsQuery = supabase.from('daily_meal_logs').select('id').eq('private_recipe_id', request.private_recipe_id);
                    } else if (request.diet_plan_recipe_id) {
                        mealLogsQuery = supabase.from('daily_meal_logs').select('id').eq('diet_plan_recipe_id', request.diet_plan_recipe_id);
                    }
                    
                    if (mealLogsQuery) {
                        const { data: logsToUpdate, error: logsError } = await mealLogsQuery;
                        if (logsError) throw logsError;

                        // 3. Update meal logs to point to the new private recipe
                        if (logsToUpdate && logsToUpdate.length > 0) {
                            const logIds = logsToUpdate.map(log => log.id);
                            const { error: updateLogsError } = await supabase
                                .from('daily_meal_logs')
                                .update({
                                    private_recipe_id: newPrivateRecipeId,
                                    diet_plan_recipe_id: null, // Ensure it points only to the new private recipe
                                })
                                .in('id', logIds);
                    ...
                    // 4. Remove original recipe from planned_meals
                    let plannedMealsQuery;
                    if (request.private_recipe_id) {
                        plannedMealsQuery = supabase.from('planned_meals').delete().eq('private_recipe_id', request.private_recipe_id);
                    } else if (request.diet_plan_recipe_id) {
                        plannedMealsQuery = supabase.from('planned_meals').delete().eq('diet_plan_recipe_id', request.diet_plan_recipe_id);
                    }
                    if (plannedMealsQuery) {
                        const { error: deletePlannedError } = await plannedMealsQuery;
                        if (deletePlannedError) console.warn("Could not delete original from planned_meals:", deletePlannedError.message);
                    }
                    ...
                    // Add the new private recipe to the plan
