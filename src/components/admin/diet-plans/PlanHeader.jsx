...
import { Users, ShieldAlert, HeartPulse, Save, User, Apple, Copy, Calendar as CalendarIcon, Tag, Edit2 } from 'lucide-react';
...
const PlanHeader = ({ plan, onUpdate, onToggleActive, readOnly = false }) => {    const navigate = useNavigate();
    ...
    const sensitivities = useMemo(() => 
        plan.sensitivities?.map(s => s.sensitivities) || [],
        [plan.sensitivities]
    );

    const conditions = useMemo(() =>
        plan.medical_conditions?.map(c => c.medical_conditions) || [],
    ...
    useEffect(() => {
        ...
        setIsNameChanged(false); // Reset on plan change
        const fetchGeneratedPlans = async () => {
            if (plan.is_template) {
                ...
                if (!error) {
                    setGeneratedPlans(data.filter(p => p.profile) || []);
                }
            }
        };
        fetchGeneratedPlans();
    ...
    const handleNameChange = (e) => {
        ...
        if (plan.is_template) {
            ...
            debounceTimeout.current = setTimeout(async () => {
                if (newName.trim() === plan.name) return;
    ...
    const saveName = async (newName) => {
        ...
        if (error) {
            toast({ title: 'Error', description: `No se pudo actualizar el nombre: ${error.message}`, variant: 'destructive' });
        } else {
    ...
    const handleSaveClick = () => {
        if (name.trim() && name.trim() !== plan.name.trim()) {
            saveName(name.trim());
        }
    };
    ...
    const handleAssignSuccess = () => {
        ...
        onUpdate(true); // Refresca los datos de la cabecera, incluyendo la lista de clientes
    };
        ...
        <>
            <Card className="bg-slate-900/50 border-gray-700 text-white overflow-hidden shadow-xl mb-8">
                <CardHeader className="relative">
                    ...
                    <div className="flex-grow">
                        ...
                        <div className="mt-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {plan.is_template ? (
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <div className="flex flex-wrap gap-1.5">
                                            {classifications.length > 0 ? (
                                                ...
                                                <span className="text-sm text-gray-500 italic flex items-center gap-1"><Tag className="w-3 h-3"/> Sin clasificar</span>
                                            )}
                                        </div>
                                    </div>
                                </div>
                                ...
                                <>
                                    <InfoItem icon={<User className="w-5 h-5 text-green-400" />} label="Cliente" asLink to={`/client-profile/${plan.user_id}`}>
                                        <p className="font-semibold text-white">{plan.profile.full_name}</p>
                                    </InfoItem>

                                    <InfoItem icon={<Apple className="w-5 h-5 text-green-400" />} label="Plan de Dieta" asLink to={`/plan/dieta/${plan.user_id}/${todayDateString}`}>
                            ...
                            <div className="space-y-4">
                                ...
                                <InfoItem icon={<HeartPulse className="w-5 h-5 text-red-400" />} label="Patologías del Plan">
                                    {conditions.length > 0 ? (
                                        <div className="flex flex-wrap gap-1.5 mt-1">
                                            {conditions.map(c => <InfoBadge key={c.id} item={c} type="medical_condition" />)}
                                        </div>
                                    ) : <p className="font-semibold text-white">Ninguna</p>}
                                </InfoItem>
                            </div>
                    ...
                    {/* 🔥 ACTION BAR (TOGGLE DEBAJO DEL TÍTULO) */}
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {generatedPlans.map(p => (
                                            <Link key={p.id} to={`/admin/manage-diet/${p.user_id}`}>
                                                <Badge variant="outline" className="text-base px-3 py-1 border-green-500/40 bg-green-900/30 text-green-300 hover:bg-green-800/40 transition-colors cursor-pointer">
                                                    {p.profile.full_name}
                                                </Badge>
                                            </Link>
                                        ))}
