...
const PlanRecipeCard = ({ recipe, onEdit, onDelete, allFoods, userRestrictions, readOnly = false }) => {
    ...
    const macros = useMemo(() => {
        return calculateMacros(ingredients, allFoods);
    ...
    const name = recipe.is_private ? recipe.name : (recipe.custom_name || "Receta sin nombre");
    ...
    const ingredientList = useMemo(() => {
        return ingredients.map(ing => {
            ...
            return (
                <span key={ing.id || `${foodId}-${ing.grams}`} className={cn(isUnsafe ? "text-red-400 font-bold" : "text-gray-400")}>
                    {text}
                </span>
            );
        });
