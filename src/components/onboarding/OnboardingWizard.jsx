...
const OnboardingWizard = ({ isOpen: propIsOpen }) => {
  ...
  // Safe check for showModal and modalContent
  const showModal = Boolean(currentStep.showModal === true && !isModalDismissed && currentStep.modalContent);
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-[#0f1115] text-white flex flex-col h-[100dvh] overflow-hidden">
      ...
      <AnimatePresence>
        {showModal && (
          <OnboardingModal
            title={currentStep.modalContent?.title || ''}
            description={currentStep.modalContent?.description || ''}
            videoUrl={currentStep.modalContent?.videoUrl}
            tips={currentStep.modalContent?.tips}
            onNext={dismissModal}
          />
        )}
      </AnimatePresence>
