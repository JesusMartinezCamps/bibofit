...
const DietObjectiveStep = ({ onNext, isLoading }) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        console.log('📥 [DietObjectiveStep] Fetching initial data... user:', user?.id);

        // Fetch types, goals, and existing user preferences in parallel
        ...
        // Preload fields if existing data found
        const pref = preferencesRes.data;
        if (pref) {
          console.log('✅ [DietObjectiveStep] Found existing preferences:', pref);

          setFormData({
            diet_goal_id: pref.diet_goal_id ? String(pref.diet_goal_id) : '',
            diet_history: pref.diet_history ?? '',
            // Convert bigint to string for Select component compatibility
            diet_type_id: pref.diet_type_id != null ? String(pref.diet_type_id) : ''
          });
        } else {
          console.log('ℹ️ [DietObjectiveStep] No existing preferences row for this user.');
          // Fields remain empty as per default state
        }

      } catch (err) {
        console.error('❌ [DietObjectiveStep] Error loading data:', err);
      } finally {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    console.log('📝 [DietObjectiveStep] Submit attempt. FormData:', formData);

    const ok = validate();
    if (!ok) {
      console.warn('⚠️ [DietObjectiveStep] Validation failed.');
      return;
    }
    ...
    console.log('✅ [DietObjectiveStep] Calling onNext with payload:', payload);
    onNext(payload);
  };
      ...
      <div className="flex-1 space-y-6 py-4 overflow-y-auto pr-2">
        ...
        <div className="space-y-3">
          ...
          <Select
            ...
            onValueChange={(v) => {
              console.log('Select Changed: Goal', v);
        ...
        <div className="space-y-3">
          ...
          <Select
            ...
            onValueChange={(v) => {
              console.log('Select Changed: Diet Type', v);
              setFormData(prev => ({ ...prev, diet_type_id: v }));
            }}
          >
