...
ALTER TABLE ONLY "public"."food_substitution_mappings"
    ADD CONSTRAINT "food_substitution_mappings_source_food_id_target_food_id_key" UNIQUE ("source_food_id", "target_food_id");
...
alter sequence "public"."joints_id_seq" owned by "public"."free_recipes"."diet_plan_id";

CREATE TRIGGER on_auth_user_created AFTER INSERT ON auth.users FOR EACH ROW EXECUTE FUNCTION public.handle_new_user();


  create policy "Allow authenticated users to upload 1rpf4wn_0"
  on "storage"."objects"
  as permissive
  for insert
  to authenticated
with check ((bucket_id = 'recipe-images'::text));
  ...
  create policy "Allow public read access 1rpf4wn_0"
  on "storage"."objects"
  as permissive
  for select
  to public
