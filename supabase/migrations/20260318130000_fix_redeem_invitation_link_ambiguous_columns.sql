...
-- Fix: "column reference center_id is ambiguous" (PostgreSQL 42702).
--
-- redeem_invitation_link uses RETURNS TABLE(..., role_id integer, center_id bigint, ...).
-- PL/pgSQL exposes RETURNS TABLE columns as implicit output variables.
-- When the function body writes ON CONFLICT (user_id, center_id) or
-- ON CONFLICT (user_id) DO UPDATE SET role_id = ..., PostgreSQL cannot
-- distinguish those column names from the same-named output variables and
-- raises 42702. Fix: replace column-list conflict targets with named
-- constraint references, which do not reference column names directly.
