-- Add tempo and target_rir to training_block_exercises
-- tempo: e.g. "3-1-2-0" (eccentric · pause-bottom · concentric · pause-top)
-- target_rir: Reps In Reserve target (0 = to failure, 1 = 1 rep short, etc.)

alter table public.training_block_exercises
  add column if not exists tempo varchar(20) default null,
  add column if not exists target_rir smallint default null
    constraint training_block_exercises_target_rir_check check (target_rir is null or target_rir between 0 and 10);

-- Also allow 1-3 day cycles (frontend caps at 7, but RPC was limiting to 4+)
