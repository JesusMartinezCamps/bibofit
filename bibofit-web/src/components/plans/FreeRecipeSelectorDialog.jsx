...
const FreeRecipeSelectorDialog = ({ open, onOpenChange, onAddNew, onRepeat }) => {
  ...
  useEffect(() => {
    ...
    const frame = requestAnimationFrame(() => {
      triggerBlock(GUIDE_BLOCK_IDS.FREE_RECIPE_SELECTOR);
    });
      ...
      <DialogContent className="bg-background border-border text-white sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription className="text-center">
            ¿Quieres crear una receta nueva desde cero o elegir una ya existente?
          </DialogDescription>
        </DialogHeader>
        <div data-guide-target="free-recipe-selector-list" className="grid gap-4 py-4">
          <button
            onClick={onAddNew}
