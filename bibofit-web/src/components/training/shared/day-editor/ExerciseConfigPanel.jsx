...
const normalizeRestSeconds = (value) => {
  ...
  if (!Number.isFinite(parsed)) return REST_DEFAULT_SECONDS;
...
const formatRestSeconds = (seconds) => {
  const safe = normalizeRestSeconds(seconds);
  const mins = Math.floor(safe / 60);
  const secs = safe % 60;
  return `${mins}:${String(secs).padStart(2, '0')}`;
};

const normalizeRepMin = (value, fallback = 8) => {
  const parsed = Number.parseInt(String(value ?? ''), 10);
  if (!Number.isFinite(parsed)) return fallback;
  return Math.max(1, Math.min(98, parsed));
};

const normalizeRepMax = (value, fallback = 12) => {
  const parsed = Number.parseInt(String(value ?? ''), 10);
  if (!Number.isFinite(parsed)) return fallback;
  return Math.max(2, Math.min(99, parsed));
};

const getRestTone = (seconds) => {
  const safe = normalizeRestSeconds(seconds);
  const ratio = (safe - REST_MIN_SECONDS) / (REST_MAX_SECONDS - REST_MIN_SECONDS);
...
const ExerciseConfigPanel = ({
  ...
  const restSeconds = normalizeRestSeconds(exercise.rest_seconds ?? exercise.restSeconds);
  const restTone = getRestTone(restSeconds);

  const handleRepMinChange = (nextValue) => {
    const nextMin = normalizeRepMin(nextValue, normalizeRepMin(exercise.target_reps_min, 8));
    const currentMax = normalizeRepMax(exercise.target_reps_max, Math.min(99, nextMin + 1));
    const nextMax = nextMin >= currentMax ? Math.min(99, nextMin + 1) : currentMax;

    onChange({
      target_reps_min: String(nextMin),
      target_reps_max: String(nextMax),
    });
  };

  const handleRepMaxChange = (nextValue) => {
    const nextMax = normalizeRepMax(nextValue, normalizeRepMax(exercise.target_reps_max, 12));
    const currentMin = normalizeRepMin(exercise.target_reps_min, Math.max(1, nextMax - 1));
    const nextMin = nextMax <= currentMin ? Math.max(1, nextMax - 1) : currentMin;
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-5 pb-2 space-y-7">
              ...
              <div className="flex flex-col items-center gap-1.5">
                ...
                <div className="flex items-center gap-1.5">
                  <Stepper
                    ...
                    min={1}
                    max={Math.max(1, normalizeRepMax(exercise.target_reps_max, 12) - 1)}
                  />
                  ...
                  <Stepper
                    ...
                    min={normalizeRepMin(exercise.target_reps_min, 8)}
                    max={99}
                  />
                </div>
              </div>
          ...
          <section>
            ...
            <p className="text-xs text-muted-foreground -mt-1 mb-4">
              Tiempo entre series efectivas para mantener rendimiento y calidad técnica.
            </p>
            <div className="rounded-2xl border border-border bg-gradient-to-br from-card to-card/60 px-4 py-4">
              ...
              <Slider
                ...
                onValueChange={([nextValue]) =>
                  onChange({ rest_seconds: normalizeRestSeconds(nextValue) })
                }
                className="mt-4"
              />

              <div className="mt-2 flex items-center justify-between text-[10px] text-muted-foreground/60 font-semibold uppercase tracking-widest">
                <span className="text-red-400">15s</span>
                <span>2:00</span>
