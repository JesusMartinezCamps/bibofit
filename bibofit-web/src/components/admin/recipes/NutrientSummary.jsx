...
const NutrientSummary = ({ macros, allVitamins, allMinerals, recipeNutrients }) => {
  const renderNutrientBadges = (allNutrients, selectedIds) => {
    if (!selectedIds || selectedIds.length === 0) {
      return <p className="text-sm text-muted-foreground italic">No se han detectado nutrientes de este tipo.</p>;
    }
    return allNutrients
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="space-y-4">
          <div>
            <h5 className="font-semibold text-muted-foreground mb-2">Vitaminas en la receta</h5>
            <div className="flex flex-wrap gap-2">
              {renderNutrientBadges(allVitamins, recipeNutrients?.vitaminIds)}
            </div>
          </div>
          <div>
            <h5 className="font-semibold text-muted-foreground mb-2">Minerales en la receta</h5>
            <div className="flex flex-wrap gap-2">
