...
const SimplifiedMineralFields = ({ allMinerals, selectedMinerals, onSelectedMineralsChange }) => {
  const handleSelectionChange = (newMineralIds) => {
    const currentMineralIds = selectedMinerals.map(m => m.mineral_id);

    // Add new minerals
    const mineralsToAdd = newMineralIds
      .filter(id => !currentMineralIds.includes(id))
      .map(id => ({ mineral_id: id, mg_per_100g: '' }));

    // Keep existing minerals that are still selected
    const keptMinerals = selectedMinerals.filter(m => newMineralIds.includes(m.mineral_id));

    onSelectedMineralsChange([...keptMinerals, ...mineralsToAdd]);
  };
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        {selectedMinerals.map(({ mineral_id, mg_per_100g }) => {
          ...
          return (
            <div key={mineral_id} className="flex items-center gap-3 p-2 bg-card/40 rounded-md border border-border/60">
...
export default SimplifiedMineralFields;