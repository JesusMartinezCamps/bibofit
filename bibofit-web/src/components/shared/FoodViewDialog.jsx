...
const FoodViewDialog = ({ open, onOpenChange, food, allVitamins, allMinerals }) => {
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="bg-background border-border text-white w-[95vw] max-w-lg">
                <DialogHeader>
                ...
                <div className="py-4 space-y-4 max-h-[70vh] overflow-y-auto no-scrollbar pr-2">
                    <div className="grid grid-cols-2 gap-4">
                        <div className="p-4 bg-muted/65 rounded-lg text-center">
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 text-sm">
                            <div className="flex items-center gap-2 p-3 bg-muted/65 rounded-lg"><CaloriesIcon className="w-5 h-5 text-orange-400" /> <span>{Math.round(macros.calories)} kcal</span></div>
                            <div className="flex items-center gap-2 p-3 bg-muted/65 rounded-lg"><ProteinIcon className="w-5 h-5 text-red-400" /> <span>{Math.round(macros.proteins)}g</span></div>
                            <div className="flex items-center gap-2 p-3 bg-muted/65 rounded-lg"><CarbsIcon className="w-5 h-5 text-yellow-400" /> <span>{Math.round(macros.carbs)}g</span></div>
                            <div className="flex items-center gap-2 p-3 bg-muted/65 rounded-lg"><FatsIcon className="w-5 h-5 text-green-400" /> <span>{Math.round(macros.fats)}g</span></div>
                        </div>
                    </div>
