...
const NutritionPanel = ({ selectedDate }) => {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8">
      ...
      {loading ? (
        <div className="text-center py-8 text-muted-foreground">Cargando plan nutricional...</div>
      ) : (
        <>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }} className="glass-effect rounded-2xl p-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                ...
                <div key={macro.label} className="bg-background rounded-lg p-4">
                  <p className="text-muted-foreground text-sm">{macro.label}</p>
                  <p className="text-2xl font-bold text-white">{Math.round(macro.current)}<span className="text-sm text-muted-foreground">/{macro.goal}{macro.unit}</span></p>
                  <div className="w-full bg-muted rounded-full h-2 mt-2"><div className="bg-[#5ebe7d] h-2 rounded-full transition-all duration-300" style={{ width: `${Math.min((macro.current / macro.goal) * 100, 100)}%` }}></div></div>
                </div>
              ))}
            </div>
          </motion.div>
          ...
          {meals.length > 0 ? (
              ...
              {meals.map((meal, index) => (
                <motion.div key={meal.id || index} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 + index * 0.1 }} className="glass-effect rounded-2xl p-6">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center space-x-3"><Clock className="w-5 h-5 text-[#5ebe7d]" /><h3 className="text-xl font-semibold text-white">{meal.name}</h3><span className="text-muted-foreground">{meal.time}</span></div>
                    <Button onClick={addMeal} size="sm" className="btn-secondary"><Plus className="w-4 h-4 mr-2" />Agregar</Button>
                  </div>
                  <div className="space-y-3">
                    {meal.foods.map((food, foodIndex) => (
                      <div key={foodIndex} className="bg-background rounded-lg p-4">
                        <div className="flex justify-between items-start">
                          <div>
                            <h4 className="font-medium text-white">{food.name}</h4>
                            <div className="flex space-x-4 text-sm text-muted-foreground mt-1"><span>{food.calories} kcal</span><span>P: {food.protein}g</span><span>C: {food.carbs}g</span><span>G: {food.fat}g</span></div>
                          </div>
                        </div>
                      </div>
                    ))}
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="glass-effect rounded-2xl p-8 text-center">
              ...
              <p className="text-muted-foreground">No hay un plan de nutrición asignado para este día.</p>
            </motion.div>
          )}
